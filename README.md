...
列车自动驾驶（ATO）车载 **MPC 控制器** 的 C++17 独立仿真包。从 TI AM5728 ATO 生产固件剥离
...
生产固件跑在 AM5728（Cortex-A15）上，直接在实车上调试控制算法代价高、风险大。本仿真包
把算法层剥离出来，用车辆动力学模型（Davis 阻力 + 旋转质量 + 坡度 + 牵引/制动曲线）做闭环
仿真，可快速调参、验证安全裕度，再迁移到生产代码。
...
**包含**：MPC 控制算法层（QP 求解、B1/B2 硬约束、自适应/分段 vref）、目标速度（GEN2 EBI/SBI）、
输出整形、停车修正、车辆动力学仿真、闭环测试、评估指标、CLI 工具。

**不含**：业务逻辑、通信（RSSP/MSCP）、ATS/TMS、车门、日志、NVRAM、DQU 数据库查询、
PID 控制器（已剥离，对比数据见上方静态表格）。这些在算法层的取值以仿真输入形式显式给出。

**与生产代码关系**：生产固件 `src/APP/`（GBK/CRLF）与本仿真包（UTF-8）是两套实现，
仿真改动不影响固件，反之亦然。
